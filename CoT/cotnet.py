...
from CoT.helpers import build_model_with_cfg
# from .layers import SelectiveKernelConv, ConvBnAct, create_attn
from CoT.registry import register_model
# from resnet import ResNet
# from CoT.layers import Shiftlution
from CoT.aggregation_zeropad import LocalConvolution
...
from CoT.layers import SplitAttnConv2d
from CoT.layers import DropBlock2d, DropPath, AvgPool2dSame, BlurPool2d, create_classifier
        ...
        if (width in conv_dim) or (width == c4_dim and block_idx not in c4_idx):
            ...
            if radix >= 1:
                self.conv2 = SplitAttnConv2d(
                    ...
                    drop_block=drop_block, act_layer=nn.SiLU)
            else:
...
def se_cotnetd_50(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 6, 3],
                      ...
                      stem_type='deep', stem_width=32, avg_down=True, base_width=64, cardinality=1, aa_layer=None,
                      # BlurPool2d
                      block_args=dict(radix=1, avd=False, avd_first=True, conv_dim={64, 128}, c4_dim=256,
...
def se_cotnetd_101(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 23, 3],
                      act_layer=nn.SiLU(inplace=True),
                      stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=None,
                      # BlurPool2d
                      block_args=dict(radix=1, avd=False, avd_first=True, conv_dim={64, 128}, c4_dim=256,
...
def se_cotnetd_152(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 8, 36, 3],
                      ...
                      block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64, 128}, c4_dim=256,
                                      c4_idx=set(range(0, 36, 2))), **kwargs)
...
def se_cotnetd_270(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[4, 29, 53, 4],
                      ...
                      stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
                      block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64, 128}, c4_dim=256,
                                      c4_idx=set(range(0, 53, 2))), **kwargs)
    return _create_se_cotnetd('cot_s', pretrained, **model_args)
